// Port verbatim (SPEC-04) de public/js/audio-guards.js.
// Comprobaciones de cordura sobre el audio ANTES de gastar cuota de STT.
...
export type AudioVerdict = { level: 'ok' | 'silent' | 'oversize'; message: string | null }

// Evalúa un blob de audio. Devuelve { level, message } donde level ∈
// 'ok' | 'silent' | 'oversize'. `seconds` es la duración estimada (puede ser 0).
export function checkAudio(sizeBytes: number, seconds: number): AudioVerdict {
  ...
  if (bytesPerSecond < MIN_BYTES_PER_SECOND) {
    return {
      level: 'silent',
      message:
        `El audio parece silencioso: ${(sizeBytes / 1024).toFixed(1)} KB para ${secs}s ` +
  ...
  if (sizeBytes > MAX_SAFE_BYTES) {
    return {
      level: 'oversize',
      message:
        `La grabación es grande (${(sizeBytes / 1024 / 1024).toFixed(1)} MB; el límite de Groq es ~25 MB). ` +
