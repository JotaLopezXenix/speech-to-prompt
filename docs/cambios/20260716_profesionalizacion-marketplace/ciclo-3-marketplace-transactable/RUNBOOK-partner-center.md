...
**Fuentes (corregido 29-jul, H-09).** Este bloque citaba «el informe de investigación», que **nunca se persistió como artefacto** en el repo — el lector no podía abrirlo. La referencia queda **sustituida por el §3 de la [`AUDITORIA-integridad-documental-2026-07-28.md`](../AUDITORIA-integridad-documental-2026-07-28.md)**, que re-verificó estos hechos contra MS Learn con las URL concretas (Subscription APIs v2, Webhook, Create plans/auto-activation, Metered billing, Workload identity federation). Añade dos hechos que aquí faltaban y que condicionan el paso G: **el metering exige el modelo *flat rate*** (no aplica a *per user*), y **el pricing model de la oferta es irreversible tras publicar**, mientras que **añadir un plan con dimensiones nuevas a una oferta flat-rate ya publicada sí está soportado**.
...
| G | **Plans & pricing** (definir Basic/Trial, precio €/mes flat, mercados, visibilidad) | decisión de negocio (precio) | 🟡 esbozado (2 planes draft). ⚠️ **Modelo sin cerrar**: a Microsoft se le dijo *pay-as-you-go*; su propuesta es **flat rate + metered billing** (dimensiones de consumo reportadas por API) — choca con el `DESIGN.md` §4 del ciclo, que deja el metering fuera de v1. **Era la 1ª pregunta del 29-jul; con la reunión pospuesta ~1 mes, pasa a decisión de negocio con la doc ya cerrada** (§5) |
...
- **[abierto, 27-jul · actualizado 29-jul] Modelo de precio sin cerrar** → bloquea el paso **G** (Plans & pricing). A Microsoft se le comunicó *pay-as-you-go*; el `DESIGN.md` §4 del ciclo deja el metered billing fuera de v1. **Decisión de negocio (Agustín + Jesús)** — y ya **no puede esperar a Microsoft**: la reunión del 29 se canceló y se retoma en ~1 mes. La doc responde lo que bloqueaba (auditoría §3.5): **nacer *flat rate* no cierra la puerta al metering** (añadir un plan con dimensiones a una oferta flat-rate publicada está soportado) y lo irreversible es el **pricing model** de la oferta. Por tanto **ya no condiciona a SPEC-03**; sí sigue condicionando SPEC-06 y el paso G.
...
- **29-jul-2026** — **La 2ª reunión (hoy, 15:30) se CANCELA y se pospone ~1 mes** (tras vacaciones). Consecuencias para este runbook: (1) las preguntas fiscales del brief (**P14/P15**: artículo del tratado España–EE. UU., asignación de payout por defecto) siguen sin interlocutor de Microsoft → **el tax del Seller no debe esperar a la reunión**, va por asesor fiscal; (2) el **modelo de precio** (paso G) pasa a ser decisión de negocio sin input externo — ver §5 con lo que la doc ya responde; (3) el brief del 29 se **recicla** para la nueva fecha. Sesión de **saneo documental** el mismo día (enmiendas de la auditoría del 28-jul): se cerró contra fuente primaria la duda de **auto-activation** y se decidió **OFF** en mesa común (ADDENDUM del `DESIGN.md` del ciclo), se corrigió la fecha del paso I (27→28-jul, H-07) y se sustituyó la referencia al informe de investigación no persistido por la auditoría §3 (H-09). La pregunta abierta sobre borrador-vs-Preview sigue viva, ahora para la reunión re-agendada.
