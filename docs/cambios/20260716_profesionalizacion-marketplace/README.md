...
| 2 — `frontend-mobile-first` | `ciclo-2-frontend-mobile-first/` | **2a diseño COMPLETO** (dirección "B · Papel") · **2b en construcción — 5/6 SPEC CERRADOS:** 01 `cimiento` ✅, 02 `api-tipada` ✅, 03 `auth-login` ✅, **04 `captura-salvaguardas` ✅** (R1, el hueso; en prod, smoke logueado incl. móvil) · **05 `resto-flujo` ✅** (Revisión/Destilado/Resultado; en prod; review limpia + smoke logueado verificado 22-jul) · **06 `marco` DESPLEGADO** (Historial/Ajustes; en `main`; review limpia, B-1/B-5 corregidos; deploy OK verificado por curl) → pendiente: **smoke logueado para CERRAR**; luego **`/jcc-spec` del cutover (SPEC-07)** cierra 2b |
