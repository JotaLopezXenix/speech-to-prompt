...
- **Nivel programa (esta carpeta):** documentos transversales a todos los ciclos.
  ...
  - `AUDITORIA-integridad-documental-2026-07-28.md` — **auditoría adversarial de integridad documental** (28-jul, sesión independiente de solo lectura): contrasta todo el árbol del programa contra código/prod/BD/Azure/MS Learn. 15 hallazgos (2 ALTA en el ADDENDUM as-built de SPEC-02 — el reproductor de audio SÍ existe en la UI y el método Reprocesar es inalcanzable —, el caso ciclo-5-sin-ADDENDUM confirmado, CLAUDE.md autocontradicho post-cutover); **cierra contra fuente primaria** auto-activation (condiciona SPEC-03), S1–S3, metering/dimensiones (añadir metering a una oferta flat-rate publicada = soportado) y S1 del programa; **confirma** provisión Azure de SPEC-02, SPEC-01 en prod y el ANALISIS al dato. Enmiendas propuestas dentro (mesa común; ninguna ejecutada).
...
| 3 — `marketplace-transactable` | `ciclo-3-marketplace-transactable/` | Diseño CERRADO. **`SPEC-01` (modelo de acceso + gate): CERRADO — desplegado y verificado en prod** (23-jul; `ALLOWED_EMAILS` retirado de Azure). **`SPEC-02` (fulfillment secretless): especificado** (27-jul), pendiente de implementar + provisión Azure. Pendiente: specs 03–06. Burocracia Partner Center **mapeada** (cuenta ✅, payout ✅, fiscal a falta de 1 acción — ver runbook); **modelo de precio sin cerrar** (pay-as-you-go vs. tarifa plana) tras la 1ª reunión de Microsoft. |
...
- **Ciclo 3** `ciclo-3-marketplace-transactable/` (arrancado 22-jul; **DESIGN cerrado**, siguiente `/jcc-spec`; track burocrático en curso):
  ...
  - `SPEC-02_fulfillment-secretless.md` — **cliente de las SaaS Fulfillment APIs sin secretos** (Fase 2, 27-jul): app de Entra dedicada + **UAMI como credencial federada** + cliente HTTP (`resolve`/`activate`/`get`/`list`/operaciones+ACK) + doble local con fixtures + script de humo. Sin dependencias nuevas ni cambios en código existente; incluye la **provisión Azure** (§6) y la **escalera de verificación** (§8.3). *(Especificado; sin implementar.)*
  ...
  - `BRIEF-reunion-isv-success-2026-07-23.md` (+ `Brief-Speech-to-prompt-ISV-Success.docx`, versión Word para compartir) — brief de la 1ª reunión de Microsoft **ISV Success** (*Discovery & Planning*, 23-jul) **+ §6 notas post-reunión** (volcadas el 27-jul de la transcripción oficial): Microsoft **recomienda fulfillment nativo**; landing y webhook simultáneos con el mismo login de compra; propuesta de **flat rate + metered billing**; la credencial federada **no llegó a tratarse**. Transcripción en `docs/reuniones/`.
  - `BRIEF-reunion-isv-success-2026-07-29.md` — brief de la **2ª reunión (29-jul 15:30, seguimiento técnico)**: modelo de precio, validación del fulfillment secretless, ciclo de vida/webhook, y las 10 preguntas que quedaron sin responder el 23.
...
- `handoffs/HANDOFF-2026-07-23.md` — **ciclo 3: DESIGN + SPEC-01 (spec→implement→review)**. `DESIGN.md` cerrado; **SPEC-01 (modelo de acceso + gate) implementado y revisado (veredicto SÍ)**, commiteado y **pusheado** (`d20d304`), **sin desplegar**. `npm test` 16/16; migración 007 idempotente; F-2 corregido; F-1 aceptado (ADDENDUM). Siguiente: deploy de SPEC-01 (migrar+seed antes) y/o `/jcc-spec` 02–06 (02/06 tras la reunión, que es **hoy**). *(Nota: los docs de la sesión llevan fecha "22-jul" por arrastre; el trabajo se commiteó el 23-jul.)*
