...
| 2 — `frontend-mobile-first` | `ciclo-2-frontend-mobile-first/` | **2a diseño COMPLETO** (dirección "B · Papel") · **2b en construcción — 5/6 SPEC CERRADOS:** 01 `cimiento` ✅, 02 `api-tipada` ✅, 03 `auth-login` ✅, **04 `captura-salvaguardas` ✅** (R1, el hueso; en prod, smoke logueado incl. móvil) · **05 `resto-flujo` ✅** (Revisión/Destilado/Resultado; en prod; review limpia + smoke logueado verificado 22-jul) · **06 `marco` IMPLEMENTADO** (Historial/Ajustes; rama `2b-06-marco`; verif. local en verde; pendiente review; cutover `/app→/` = SPEC-07/RUNBOOK aparte) → siguiente: **`/jcc-review` del SPEC-06** |
