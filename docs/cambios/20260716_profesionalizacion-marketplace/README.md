...
| 2 — `frontend-mobile-first` | `ciclo-2-frontend-mobile-first/` | **2a diseño COMPLETO** (dirección "B · Papel") · **2b en construcción — 4/6 SPEC CERRADOS:** 01 `cimiento` ✅, 02 `api-tipada` ✅, 03 `auth-login` ✅, **04 `captura-salvaguardas` ✅** (R1, el hueso; en prod, smoke logueado incl. móvil) · **05 `resto-flujo` DESPLEGADO** (Revisión/Destilado/Resultado; en `main`; review adversarial veredicto SÍ, 2 BAJA corregidos; deploy OK, bundle `index-bVTGOk0U.js` verificado por curl) → pendiente: **smoke logueado del usuario para CERRAR**; luego **`/jcc-spec` del SPEC-06** |
