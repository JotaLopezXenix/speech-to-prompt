...
| 2 — `frontend-mobile-first` | `ciclo-2-frontend-mobile-first/` | **2a diseño COMPLETO** (dirección "B · Papel") · **2b — 6/6 SPEC CERRADOS (queda el cutover):** 01 `cimiento` ✅, 02 `api-tipada` ✅, 03 `auth-login` ✅, **04 `captura-salvaguardas` ✅** (R1, el hueso; en prod, smoke logueado incl. móvil) · **05 `resto-flujo` ✅** (Revisión/Destilado/Resultado; en prod; review limpia + smoke logueado verificado 22-jul) · **06 `marco` ✅** (Historial/Ajustes; en prod; review limpia + smoke logueado verificado 22-jul) · **cutover `/app→/` (SPEC-07) DESPLEGADO** (en `main`; review limpia + MEDIA corregida; deploy OK, `/` sirve el SPA nuevo, `/app`→302→`/`, curl verificado) → pendiente: **smoke logueado en `/` para CERRAR 2b** |
