...
- **Fase actual:** Trabajo activo = programa **`profesionalizacion-marketplace`** (multi-ciclo), **ciclo 2 `frontend-mobile-first` · sub-ciclo 2b (construcción)**. Estado: **SPEC-01…05 CERRADOS y en producción** (01 cimiento · 02 API tipada + alias `/api/v1` · 03 auth/login MSAL · 04 captura + salvaguardas R1 · 05 resto del flujo Revisión/Destilado/Resultado —review adversarial limpia + **smoke logueado del usuario verificado 22-jul**, incl. happy-path del LLM y copiado). **SPEC-06 `marco` DESPLEGADO** (Historial/Ajustes reales; en `main`, review adversarial limpia —veredicto SÍ, B-1/B-5 corregidos—; merge ff + deploy OK; **bundle `index-CtDRVbkq.js` verificado por curl == build local**, `/app/history` y `/app/settings` 200, `/api/v1` vivo, viejo `/` intacto). **Pendiente SOLO el smoke logueado del usuario para CERRAR. Siguiente = smoke logueado; luego `/jcc-spec` del cutover `/app→/` (SPEC-07/RUNBOOK, encuadrado en `SPEC-06 §7`) cierra el ciclo 2b.** — Mapa del cambio activo: [`README`](docs/cambios/20260716_profesionalizacion-marketplace/README.md) · última bitácora: [`handoffs/HANDOFF-2026-07-21-spec04.md`](docs/cambios/20260716_profesionalizacion-marketplace/handoffs/HANDOFF-2026-07-21-spec04.md) · historia completa de TODOS los cambios: [`índice global docs/cambios/README.md`](docs/cambios/README.md). — *Esta línea es SOLO un puntero al trabajo ACTIVO y se sobrescribe (NO acumula historia). El detalle de cada cambio —activo o cerrado— vive en su carpeta `docs/cambios/<fecha>_<slug>/` (con su README/DESIGN/SPEC) y la historia fechada con evidencia en sus `handoffs/`. Reconciliación: contrasta esta línea con los artefactos reales del repo al arrancar.*
