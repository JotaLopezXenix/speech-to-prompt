...
- **Fase actual:** cambio `mejorar-destilado-gpt` **COMPLETO y desplegado a producción** (26-jun-2026; `docs/cambios/20260626_mejorar-destilado-gpt/`). Decisión: el modo `completo` sigue siendo **brief en 1ª persona, cerrado y denso** (se **revirtió** la reescritura neutra/`[inferido]`/preguntas-abiertas de la 1ª iteración —era errónea, colapsaba `completo`≈`limpio`—). `completo.md` reescrito **afinado para GPT** (corrige nombres mal transcritos, densidad sin perder datos; salida destinada a Claude Code) — origen `eval/prompts/completo-gpt-v3.md`. **Modelo elegido `gpt-4.1`** vía evaluación empírica contra el golden Sonnet (`data/sessions/2026-06-06T10-12-24.json`) sobre 5 sesiones, con `scripts/eval-distill.mjs` (artefactos en `eval/`); principio coste-primero. **Migración 005 (catálogo de candidatos) ELIMINADA** — `gpt-4.1` (de la 004) basta; lista de modelos del provider recortada a los reales; `max_completion_tokens` en provider se mantiene. **Desplegado a producción:** push a `main`→GitHub Actions; `seed-prompts` del prompt v3 contra la BD Azure (`SQL_AUTH=entra-default`+`az login`; admin Entra=jesus.lopez); App Service reiniciado. **Verificado en prod:** smoke test OK (sesión id 6: `gpt-4.1`/`completo`/prompt v3, 1ª persona). Bitácora de cierre: `docs/cambios/20260626_mejorar-destilado-gpt/20260626_Cierre_Sesion.md`. **Residuo menor opcional:** añadir "prompt" a la corrección de nombres del prompt `completo`. — Cambio anterior `azure-sql-multiusuario` **COMPLETO** en producción (flujos 1-6, secretless, red privada; `docs/cambios/20260623_azure-sql-multiusuario/`). **Pendientes menores** (no núcleo): limpiar UI de Ajustes (proveedores legacy) y añadir IP de Agustín al firewall.
