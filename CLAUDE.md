...
- **Fase actual:** Trabajo activo = programa **`profesionalizacion-marketplace`** (multi-ciclo), **ciclo 2 `frontend-mobile-first` · sub-ciclo 2b (construcción)**. Estado: **SPEC-01…06 CERRADOS y en producción** (01 cimiento · 02 API tipada + alias `/api/v1` · 03 auth/login MSAL · 04 captura + salvaguardas R1 · 05 resto del flujo Revisión/Destilado/Resultado · 06 marco Historial/Ajustes; los seis con review adversarial limpia + **smoke logueado del usuario verificado**). El **ciclo 2b tiene sus 6 SPEC cerrados**; el **`cutover /app→/` (SPEC-07) está IMPLEMENTADO + REVISADO LIMPIO** (rama `2b-07-cutover`; `vite base`+PWA a `/`, `web/dist` servido en `/` + redirect transitorio `/app→/` 302, `public/` retirado; review adversarial independiente = **veredicto SÍ**, 1 MEDIA —bucle 302 en `/app?query`— **corregida** + 2 BAJA aceptadas; build/lint/14-14 + routing local verde). **Pendiente para CERRAR: merge + deploy + smoke logueado en `/`** (precondición: SPA redirect URI raíz en Entra; rollback = revertir + redeploy). **Siguiente = merge+deploy (con OK del usuario) + smoke**; con eso **el ciclo 2b queda cerrado**. — Mapa del cambio activo: [`README`](docs/cambios/20260716_profesionalizacion-marketplace/README.md) · última bitácora: [`handoffs/HANDOFF-2026-07-21-spec04.md`](docs/cambios/20260716_profesionalizacion-marketplace/handoffs/HANDOFF-2026-07-21-spec04.md) · historia completa de TODOS los cambios: [`índice global docs/cambios/README.md`](docs/cambios/README.md). — *Esta línea es SOLO un puntero al trabajo ACTIVO y se sobrescribe (NO acumula historia). El detalle de cada cambio —activo o cerrado— vive en su carpeta `docs/cambios/<fecha>_<slug>/` (con su README/DESIGN/SPEC) y la historia fechada con evidencia en sus `handoffs/`. Reconciliación: contrasta esta línea con los artefactos reales del repo al arrancar.*
