...
- **Fase actual:** cambio `mejorar-destilado-gpt` en **implementación** (`docs/cambios/20260626_mejorar-destilado-gpt/`). SPEC-01: reescribir `openai/completo.md` (neutro, `[inferido]`, preguntas abiertas). SPEC-02: migración `005_llm_models_candidates.sql` (o3/o3-pro/o4-mini/gpt-5/gpt-5.1/gpt-5.4, `enabled=0`) + `max_completion_tokens` en provider. Siguiente paso: `/jcc-implement`. — Cambio anterior `azure-sql-multiusuario` **COMPLETO** en producción (flujos 1-6, secretless, red privada; `docs/cambios/20260623_azure-sql-multiusuario/`). **Pendientes menores** (no núcleo): limpiar UI de Ajustes (proveedores legacy) y añadir IP de Agustín al firewall.
