...
- **Fase actual:** Programa **`profesionalizacion-marketplace`** (multi-ciclo). **Ciclo 3 `marketplace-transactable` ACTIVO — JCC Fase 1 (diseño) EN CURSO/PAUSADA** a la espera de la reunión con **Microsoft ISV Success (23-jul)**; **aún SIN `DESIGN.md` del código** (entrevista socrática a medias). En paralelo, **track burocrático** de Partner Center abierto (`ciclo-3-marketplace-transactable/RUNBOOK-partner-center.md`; alta **mapeada**: cuenta+payout ✅, fiscal a falta de 1 acción en el tax del Seller). **Siguiente = continuar `/jcc-design`** (2ª tanda, decisiones estructurales a la mesa común: comprador individual/empresa · autoservicio/alta manual · reconciliación identidad-comprador · punto del gate · **tensión secretless del fulfillment**) → luego `/jcc-spec`. (Ciclos 0–2 cerrados y en prod.) El gate por suscripción sustituirá la lista blanca `ALLOWED_EMAILS` (muerde el hueco H6 `users.email NOT NULL`). Pendientes cross-cutting y toda la evidencia, en los handoffs. — Mapa del cambio activo: [`README`](docs/cambios/20260716_profesionalizacion-marketplace/README.md) · última bitácora: [`handoffs/HANDOFF-2026-07-22-ciclo3-arranque.md`](docs/cambios/20260716_profesionalizacion-marketplace/handoffs/HANDOFF-2026-07-22-ciclo3-arranque.md) · historia completa de TODOS los cambios: [`índice global docs/cambios/README.md`](docs/cambios/README.md). — *Esta línea es SOLO un puntero al trabajo ACTIVO y se sobrescribe (NO acumula historia). El detalle de cada cambio —activo o cerrado— vive en su carpeta `docs/cambios/<fecha>_<slug>/` (con su README/DESIGN/SPEC) y la historia fechada con evidencia en sus `handoffs/`. Reconciliación: contrasta esta línea con los artefactos reales del repo al arrancar.*
