...
- **Fase actual:** Trabajo activo = programa **`profesionalizacion-marketplace`** (multi-ciclo), **ciclo 2 `frontend-mobile-first` · sub-ciclo 2b (construcción)**. Estado: **SPEC-01…04 CERRADOS y en producción** (01 cimiento · 02 API tipada + alias `/api/v1` · 03 auth/login MSAL · 04 captura + salvaguardas R1); **SPEC-05 `resto-flujo` IMPLEMENTADO + REVISADO LIMPIO** (Revisión/Destilado/Resultado reales; rama `2b-05-resto-flujo`; review adversarial independiente = **veredicto SÍ**, 0 ALTA/0 MEDIA, 2 BAJA ya corregidos; build/lint/14-14 verde). **Pendiente para CERRAR: merge a `main` + deploy + smoke logueado en Azure** (el happy-path del LLM y el copiado al portapapeles no son verificables en local por la red privada de Azure OpenAI). **Siguiente = desplegar + smoke logueado; luego `/jcc-spec` del SPEC-06** (marco: Historial/Ajustes) y el **cutover final `/app→/`**. — Mapa del cambio activo: [`README`](docs/cambios/20260716_profesionalizacion-marketplace/README.md) · última bitácora: [`handoffs/HANDOFF-2026-07-21-spec04.md`](docs/cambios/20260716_profesionalizacion-marketplace/handoffs/HANDOFF-2026-07-21-spec04.md) · historia completa de TODOS los cambios: [`índice global docs/cambios/README.md`](docs/cambios/README.md). — *Esta línea es SOLO un puntero al trabajo ACTIVO y se sobrescribe (NO acumula historia). El detalle de cada cambio —activo o cerrado— vive en su carpeta `docs/cambios/<fecha>_<slug>/` (con su README/DESIGN/SPEC) y la historia fechada con evidencia en sus `handoffs/`. Reconciliación: contrasta esta línea con los artefactos reales del repo al arrancar.*
