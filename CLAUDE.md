...
- **Fase actual:** cambio `mejorar-destilado-limpio` **IMPLEMENTADO y validado por eval, pendiente de desplegar a prod** (28-jun-2026; `docs/cambios/20260628_mejorar-destilado-limpio/`; DESIGN + SPEC.md). `src/prompts/openai/limpio.md` reescrito **afinado para GPT** (origen `eval/prompts/limpio-gpt-v3.md`): mantiene el contrato `limpio` (no resolver/no sintetizar/`[inferido]`/sección "❓ Preguntas abiertas"/no corregir nombres en silencio) y añade **un cambio de comportamiento: preservar la voz/persona del hablante** (1ª persona si el dictado lo es; ya no neutraliza a impersonal). Endurecidos para GPT los límites que se saltaba: marcar palabras mal transcritas con `[inferido]` (no dejarlas crudas ni corregirlas en silencio), no inflar preguntas con agenda, no filtrar las propias instrucciones como contenido. **Modelo `gpt-4.1` se mantiene.** Validado por eval contra golden fabricado (Session 10, transcripción del dictado más largo en prod) con `scripts/eval-distill.mjs` (override nuevo `EVAL_OUT_DIR`); v1→v2→v3→**v4** (la **review adversarial independiente** halló en v3 una pregunta-agenda sintetizada [S1] y una pasiva [V1]; v4 las corrige). **v4 pasa los 7 criterios** (SPEC §5.3); `REVIEW.md` documenta el bucle 3↔4. Prompt runtime = `eval/prompts/limpio-gpt-v4.md`. `seed-prompts` a la **DB local** OK (no a prod). **La eval requirió abrir temporalmente el acceso público de `aoai-speech-to-prompt` (red privada) y re-cerrarlo** (`publicNetworkAccess` quedó `Disabled`). **Pendiente:** desplegar a prod (push→Actions + `seed-prompts` contra Azure + reinicio App Service) — decisión aparte. **Aparcado:** bug de grabación que se detiene sola. — Cambio anterior `mejorar-destilado-gpt` **COMPLETO y desplegado a producción** (26-jun-2026; `docs/cambios/20260626_mejorar-destilado-gpt/`): `completo` afinado para GPT (1ª persona, cerrado, denso, corrige nombres en silencio), modelo `gpt-4.1` elegido por eval contra golden Sonnet sobre 5 sesiones (coste-primero); migración 005 eliminada. **Residuo menor opcional:** añadir "prompt" a la corrección de nombres del prompt `completo`. — Cambio `azure-sql-multiusuario` **COMPLETO** en producción (flujos 1-6, secretless, red privada; `docs/cambios/20260623_azure-sql-multiusuario/`). **Pendientes menores** (no núcleo): limpiar UI de Ajustes (proveedores legacy) y añadir IP de Agustín al firewall. Mismo método que `completo` aplicado a `limpio`, pero **alcance ligero** (el usuario considera que `limpio` funciona bien): afinado del prompt para GPT (endurecer límites que GPT se salta "por ayudar": resolver ambigüedades, sintetizar, opinar, inflar preguntas) + **un único cambio de comportamiento (estructural, aprobado): preservar la voz/persona del hablante** en vez de neutralizar a impersonal ("se quiere"→queda en 1ª persona si el speech es en 1ª). Se mantiene intacto el resto del contrato `limpio` (no resolver/no sintetizar/`[inferido]`/sección "❓ Preguntas abiertas"/no corregir nombres en silencio). **Golden fabricado** sobre la transcripción de **Session 10** de prod (no hay golden `limpio` reutilizable: única sesión local en `limpio` es trivial); Claude genera la referencia bajo el spec nuevo, se afina GPT hasta igualarla, reusando `scripts/eval-distill.mjs` (`EVAL_PROMPT=src/prompts/openai/limpio.md`). **Modelo `gpt-4.1` se mantiene** (no se re-evalúa salvo que no llegue). Solo toca `src/prompts/openai/limpio.md` (sin código runtime). **Fuera:** otros modos, familia claude, UI, despliegue (decisión al cierre), y el **bug de grabación que se detiene sola** (aparcado; evidenciado en la propia transcripción de Session 10; sin temporizador de silencio/maxDuration en `audio-recorder.js`, el stop viene de fuera). — Cambio anterior `mejorar-destilado-gpt` **COMPLETO y desplegado a producción** (26-jun-2026; `docs/cambios/20260626_mejorar-destilado-gpt/`): `completo` afinado para GPT (1ª persona, cerrado, denso, corrige nombres en silencio), modelo `gpt-4.1` elegido por eval contra golden Sonnet sobre 5 sesiones (coste-primero); migración 005 eliminada. **Residuo menor opcional:** añadir "prompt" a la corrección de nombres del prompt `completo`. — Cambio `azure-sql-multiusuario` **COMPLETO** en producción (flujos 1-6, secretless, red privada; `docs/cambios/20260623_azure-sql-multiusuario/`). **Pendientes menores** (no núcleo): limpiar UI de Ajustes (proveedores legacy) y añadir IP de Agustín al firewall.
